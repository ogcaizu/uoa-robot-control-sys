...
int main(int argc, char** argv) {
  ...
  while (ros::ok()) {
	...
	int decode_x_ftoi, decode_y_ftoi;
	...
	//天井の高さごとに重みKu,Kvを割り振る
	decode_x_ftoi = (int)(1000 * decode_x);
	decode_y_ftoi = (int)(1000 * decode_y);
	if ((decode_x_ftoi==0 && decode_y_ftoi==0)||(decode_x_ftoi==1494 && decode_y_ftoi==0)||(decode_x_ftoi==712 && decode_y_ftoi==1500)) {
	Ku = 0.0030743;
	Kv = 0.00315636;
	}
	else {
